...
- README.md 작성: 기능 소개, 설치·실행 방법, 사용 흐름, 명명 규칙, 동작 원리

### 3. UX 개선 (v0.2.0)

- 이미지 목록: `Listbox` → `ttk.Treeview` + 백그라운드 썸네일 로드 (64×48px)
- 목록 클릭 시 해당 이미지로 이동 + AI 분석 자동 시작
- 키보드 단축키: `Enter` 저장 / `Space` 건너뜀 / `B` 뒷면추가 (Entry 포커스 시 무시)
- 목록 자동 스크롤: 진행 시 현재 항목이 항상 보이도록 `tree.see()` 적용
- `_analysis_gen` 세대 번호: 목록 빠르게 전환 시 이전 분석 결과 무시
