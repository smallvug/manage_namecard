...
| 기능 | 설명 |
|------|------|
| **AI 명함 인식** | Claude Vision(`claude-opus-4-6`)이 이름·기관·직책을 읽어 폴더명 생성 |
...
_data/ 에 명함 사진 넣기
          ↓
프로그램 실행 → 왼쪽 목록에서 사진 선택
          ↓
[분석] 버튼 클릭 → Claude AI가 폴더명 자동 입력
          ↓
필요하면 폴더명 수정 → [저장] 클릭
          ↓
...
| 버튼 | 동작 |
|------|------|
| **분석** | AI로 명함 인식 후 폴더명 자동 입력 (앞뒷면 감지 동시 수행) |
| **저장** | 입력된 폴더명으로 `_processed/` 에 이동 |
| **뒷면 추가** | 기존 폴더를 선택해 현재 사진을 뒷면으로 추가 |
| **건너뜀** | 현재 사진을 분류하지 않고 다음으로 넘김 |
